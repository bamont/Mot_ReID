...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--pool-dir", type=Path, required=True, help="Dossier du pool (contient manifest.csv)"
    )
    parser.add_argument(
        "--fold-root", type=Path, default=None, help="Ou ecrire les folds (defaut: a cote du pool)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Defaut auto selon le modele (voir train.py) si non fourni",
    )
    parser.add_argument(
        "--lr0",
        type=float,
...
def main() -> None:
    ...
    with args.summary_csv.open("w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["held_out", "epoch", "precision", "recall", "mAP50", "mAP50-95"]
        )
        writer.writeheader()
